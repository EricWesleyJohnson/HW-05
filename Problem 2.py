...
def main():  # Plain stress approximation
    ...
    stress_resultant = np.array([[1000], [0], [0], [0], [5], [0]])

    N = 12                   # number of plies
    ...
    angle_in_degrees = [0, 30, 60, 90, -60, -30, -30, -60, 90, 60, 30, 0]
    ...
    M_MS_xxc = [0] * N
    for i in range(N):
        M_MS_xxc[i] = R_MS[i] * stress_resultant[4]
    ...
    print("M_yy = " + str(np.round(abs(min(M_MS_xxc[0], M_TW_xxc[0])), 2)))
